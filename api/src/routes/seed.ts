...
async function seedDatabase(): Promise<void> {
  ...
  const tickets = await Promise.all([
    prisma.ticket.create({
      data: {
        ticketThreadId: 'ticket-001',
        guildId: guildId,
        type: 'VERIFICATION_ID',
        status: 'OPEN',
        title: 'ID Verification Request',
        openedById: members[0].id,
      },
    }),
    prisma.ticket.create({
      data: {
        ticketThreadId: 'ticket-002',
        guildId: guildId,
        type: 'STAFF_TALK',
        status: 'IN_PROGRESS',
        title: 'Need to discuss moderation policy',
        openedById: members[1].id,
      },
    }),
    prisma.ticket.create({
      data: {
        ticketThreadId: 'ticket-003',
        guildId: guildId,
        type: 'EVENT_REPORT',
        status: 'AWAITING_RESPONSE',
        title: 'Weekly event report - March 2024',
        openedById: members[2].id,
      },
    }),
    prisma.ticket.create({
      data: {
        ticketThreadId: 'ticket-004',
        guildId: guildId,
        type: 'UNSOLICITED_DM',
        status: 'CLOSED',
        title: 'Received unsolicited DM from user',
        openedById: members[3].id,
    ...
    prisma.ticket.create({
      data: {
        ticketThreadId: 'ticket-005',
        guildId: guildId,
        type: 'FRIEND_REQUEST',
        status: 'OPEN',
        title: 'Friend request from unknown user',
        openedById: members[4].id,
      },
    }),
    prisma.ticket.create({
      data: {
        ticketThreadId: 'ticket-006',
        guildId: guildId,
        type: 'DRAMA',
        status: 'IN_PROGRESS',
  ...
  await Promise.all([
    prisma.verificationTicket.create({
      data: {
        ...
        initialVerifierId: staffMember.id,
    ...
    prisma.verificationTicket.create({
      data: {
        ...
        initialVerifierId: staffMember.id,
        finalVerifierId: staffMember.id,
